...
async function run() {
	...
	for (const fileName of files) {
		...
		for (const dept in unfilteredOfferings) {
			if (TARGET_DEPTS.includes(dept)) {
				...
				if (1 > 0 || !fetched) {
					...
					// 			cookie: "PHPSESSID=5bphf2bgl66sgdelk9sl7as2vn",
					...
					const resp = await fetch(
						`https://stars.bilkent.edu.tr/homepage/ajax/plainOfferings.php?COURSE_CODE=${dept}&SEMESTER=${currSemester}&submit=List%20Selected%20Offerings&rndval=${new Date().getTime()}`,
						{
							headers: {
								...
								"sec-fetch-mode": "cors",
								"sec-fetch-site": "same-origin",
								cookie: "authchallenge=fdcdfc9d7717bccd266e603c4d4a62d2; PHPSESSID=5bphf2bgl66sgdelk9sl7as2vn",
								Referer: "https://stars.bilkent.edu.tr/homepage/plain_offerings",
								"Referrer-Policy": "strict-origin-when-cross-origin",
							},
							body: undefined,
					...
					fs.writeFileSync(path.join(__dirname, fileName.replace(".json", "offerings2.html")), textResp);
		...
		fs.writeFileSync(
			path.join(__dirname, fileName.replace(".json", "grouped.json")),
			JSON.stringify(filteredOfferings, null, 4)
		);

		const easier = Object.values(filteredOfferings).reduce((cum, curr) => {
			cum.push(Object.values(curr));
			return cum.flat();
		}, [] as any[]);

		fs.writeFileSync(
			path.join(__dirname, fileName.replace(".json", "easier.json")),
			JSON.stringify(easier, null, 4)
		);
